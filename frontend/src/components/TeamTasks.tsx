          ...
          {taskGroups.map((group) => {
            ...
            return (
              ...
              >
                {/* 팀원 카드 헤더 */}
                <div
                  onClick={hasTasks ? () => handleToggleMember(group.userId) : undefined}
                  className={`p-4 ${hasTasks ? 'cursor-pointer hover:bg-gray-50' : ''} transition-colors`}
                  role={hasTasks ? 'button' : undefined}
                  tabIndex={hasTasks ? 0 : undefined}
                  onKeyDown={hasTasks ? (e) => {
